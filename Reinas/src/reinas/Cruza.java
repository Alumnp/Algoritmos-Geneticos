...
public class Cruza {
    ...
    public static Individuo porMascara(int[] mascara, 
        ...
        // Creamos el genotipo del hijo de forma 
        //que no se repita ningun valor en su genotipo
        for (int i = 0; i < genPadre.length; i++) {
            genHijo1[i] = i;
            genHijo2[i] = i;
        }

        // Aqui se hace la cruza, de manera que no se repita 
        //ninguna posicion de las reinas
        int aux, pos;
        for (int i = 0; i < genPadre.length; i++) {
            if (mascara[i] == 0) {
